...
class Messages extends React.Component{
    ...
    render(){
        const currentUserEmail = JSON.parse(this.props.currentUser).email
       let users = _.uniqBy(this.props.messages,'id');
       console.log(users);
       users = users.filter(user=>user.id!==JSON.parse(this.props.currentUser).id && user.name !=="")
       console.log(users)
    ...
    return <ListGroup> 
        {users.map(user=><Link to={{pathname:"/chat", state:{receiver:user.email}}}> <ListGroupItem>{user.name}</ListGroupItem>
        </Link>)}
        </ListGroup>
    }
}
