...
const slice = createSlice({
    ...
    reducers: {
        ...
        selectedConversation: (state, action) => {
            state.selectedConversation = state.conversations.find(item => item.id === action.payload)
        },

        newMessageAdded: (state, action) => {
            const currentUserId = JSON.parse(localStorage.getItem("user")).id
            const sender_index = state.conversations.findIndex(item => (item.id === action.payload.sender));
            const receiver_index = state.conversations.findIndex(item => (item.id === action.payload.receiver));
            if (sender_index >= 0 && ((currentUserId === action.payload.sender) || (currentUserId === action.payload.receiver))) {
            ...
            if (receiver_index >= 0 && ((currentUserId === action.payload.sender) || (currentUserId === action.payload.receiver))) {
                state.conversations[receiver_index].last_message = action.payload;


            }
            if (state.selectedConversation) {
                if (((state.selectedConversation.id === action.payload.receiver)
                    || (state.selectedConversation.id === action.payload.sender))
                    && ((currentUserId === action.payload.sender) || (currentUserId === action.payload.receiver))
                ) {
        ...
        messagesReceived: (state, action) => {
            ...
            if (action.payload != null) {
                const index = state.conversations.findIndex(item => (item.id === action.payload[0].sender) || (item.id === action.payload[0].receiver))
...
export const loadConversations = (id) => (dispatch) => {
    ...
    return axios.get(`${rootApi}/user-list/?id=${id}`).then(response => {
        return dispatch({
...
export const messagesRequested = (sender, conversationId) => (dispatch) => {
    return axios.get(`${rootApi}/chat-history/?sender=${sender}&receiver=${conversationId}`).then(response => {
        return dispatch({
...
export const filterConversationById = createSelector(
    ...
    (chats) => chats.conversations.filter(chat => chat.id === chats.conversationId)
)
